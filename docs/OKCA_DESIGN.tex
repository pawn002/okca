...
WCAG~2.x contrast
$\bigl(\text{ratio} = (Y_1 + 0.05)\,/\,(Y_2 + 0.05)\bigr)$
has two well-documented failure modes:
...
\begin{enumerate}
  ...
  \item \textbf{False passes for green text/backgrounds near the AA
    boundary.}
    OKLCH~$L^3$ (used as a perceptually uniform luminance proxy)
    underestimates WCAG~$Y$ for green hues because IEC~61966-2-1 weights
    sRGB green strongly ($0.7152$).  A pair that WCAG rates $4.4$ can
    appear to score $4.7$ when the luminance proxy is miscalibrated.
\end{enumerate}

OKCA corrects both, while maintaining \textbf{zero false passes} against
WCAG---meaning OKCA never tells a designer a pair is safe when WCAG says
it fails.

%% =====================================================================
\section{Core Design Principles}

\subsection{Safety: FP\,=\,0 Is Non-Negotiable}
...
OKCA outputs ratios in $[1,\,21]$, uses the same AA threshold ($4.5$)
and AAA threshold ($7.0$) as WCAG~2.x.  Designers and developers see
familiar numbers.  No re-education required.
...
colour space.  For neutral greys, $L^3 \approx \text{WCAG}\;Y$---the
cube-root perceptual transform inverts to linear luminance.  Using $L$
...
%% =====================================================================
\section{Why $L^3$ Instead of WCAG $Y$}
...
\begin{enumerate}
  \item \textbf{Green is over-weighted.}  The $0.7152$ green coefficient
    ...
    effectiveness.  Pure green at WCAG~$Y = 0.715$ scores $14.5{:}1$
    against black---visually plausible---but formula-level deviations
    accumulate near the AA boundary for dark greens.
  ...
  \item \textbf{Chromatic text on dark backgrounds.}  WCAG~$Y$ does not
    ...
    chromatic lighter element at the same $Y$ as an achromatic one
    produces the same ratio---even though the saturated one has
    demonstrably lower effective contrast against a dark background.
\end{enumerate}

OKLCH~$L^3$ addresses points~2 and~3 directly.  The green
over-weighting (point~1) is corrected by step~4 of the algorithm
(\S\ref{sec:green}).

%% =====================================================================
\section{The FP\,=\,0 Guarantee}
\label{sec:fp0}

The guarantee rests on two properties of the algorithm:

\paragraph{Step~3 (lighter-element compression) can only reduce the ratio.}

\begin{lstlisting}
exp = 1 + CHROMA_K * satW    where satW in [0, 1]
\end{lstlisting}

For achromatic colours, $\mathit{satW} = 0$, $\mathit{exp} = 1$, and
$\mathit{lighterY} = L^3$---the pure luminance proxy.  For saturated
colours, $\mathit{exp} > 1$, so
$L_{\mathrm{lighter}}^{\,\mathit{exp}} < L_{\mathrm{lighter}}$ (since
$L \le 1$), making
$\mathit{lighterY} < L_{\mathrm{lighter}}^{\,3} \le Y_{\mathrm{WCAG,lighter}}$.
The compressed lighter element produces a \emph{smaller} numerator and
therefore a \emph{lower} ratio than WCAG.

\paragraph{Step~4 (darker-element green correction) can only increase
$\mathit{darkerY}$.}

The correction adds a non-negative term to $\mathit{darkerL}$ before
cubing.  A larger $\mathit{darkerY}$ means a larger denominator and
therefore a \emph{lower} ratio than the uncorrected version.  The gate
$a < -A_{\mathrm{THRESH}}$ ensures it fires only where $L^3$ genuinely
undershoots WCAG~$Y$ (true greens), preventing spurious ratio reductions
elsewhere.

\medskip
Together:
\[
  \text{ratio}_{\mathrm{OKCA}} \;\le\; \text{ratio}_{\mathrm{WCAG}}
  \qquad\text{for any input.}
\]
A pair WCAG fails ($\text{ratio} < 4.5$) will also fail OKCA.
...
\begin{lstlisting}
C    = sqrt(a^2 + b^2)            // Oklab chroma of lighter element
satW = min(1, (C / 0.15)^2)       // quadratic ramp: 0 at C=0, 1 at C>=0.15
exp  = 1 + 0.75 * satW            // range: 1.0 (achromatic) ... 1.75 (saturated)
lighterY = (lighterL ^ exp) ^ 3
\end{lstlisting}

For a neutral white lighter element: $C = 0$, $\mathit{satW} = 0$,
$\mathit{exp} = 1$, $\mathit{lighterY} = L^3 = 1.0$ (unchanged).

For hot pink ($C \approx 0.197 > 0.15$): $\mathit{satW} = 1$,
$\mathit{exp} = 1.75$---the lighter element is penalised and the ratio
drops below WCAG.

The threshold $C_{\mathrm{THRESH}} = 0.15$ is calibrated so the penalty
is negligible for lightly tinted neutrals (e.g.\ off-white at
$C \approx 0.01$) and fully active for vivid designer palette colours
($C \ge 0.15$).
...
\paragraph{Problem.}
For green hues, OKLCH~$L^3$ underestimates WCAG~$Y$.  Specifically,
...
\paragraph{Mechanism.}
For the darker element, if its Oklab $a$ coordinate is below
$-A_{\mathrm{THRESH}}$ (true greens; not blues, which have a slightly
negative $a$ but at much smaller magnitude):
...
$= 0.155 \times (0.135 - 0.05) = 0.013$, $L_{\mathrm{eff}} = 0.558 +
0.013 = 0.571$, $\mathit{darkerY} = 0.186$.  The inflated denominator
brings the ratio down from $4.5$ to $4.4$---matching WCAG's $4.4$.

\paragraph{The $A_{\mathrm{THRESH}}$ gate matters.}
...
$L^3 \approx Y_{\mathrm{WCAG}}$ and no correction is needed.  Setting
$A_{\mathrm{THRESH}} = 0.05$ restricts the correction to colours where
the $L^3 / Y_{\mathrm{WCAG}}$ gap is real and meaningful.

\paragraph{$K_{\mathrm{DARK}}$ and rounding.}
$K_{\mathrm{DARK}} = 0.155$ gives a raw ratio of $4.448$ for
white/\texttt{\#228b22}, which rounds to $4.4$ via
\texttt{toFixed(1)}.  Probe calibration must account for the production
rounding mode---a raw ratio of $4.471$ would round to ``4.5'',
producing a false pass at display precision.
...
\begin{itemize}
  \item \textbf{Step~3:} Oklab chroma $C = 0 \;\Rightarrow\;
    \mathit{satW} = 0 \;\Rightarrow\; \mathit{exp} = 1 \;\Rightarrow\;
    \mathit{lighterY} = L^3$.
  \item \textbf{Step~4:} Oklab $a = 0 \;\Rightarrow\;$ gate not
    reached $\;\Rightarrow\; \mathit{darkerY} = L^3$.
\end{itemize}

$L^3 \approx Y_{\mathrm{WCAG}}$ for greys (up to floating-point
precision of the OKLCH transform).  The algorithm therefore produces
\textbf{exact WCAG~2.x ratios for achromatic pairs}---white/black $=
21$, white/\texttt{\#767676} $= 4.5$.  This is the most important
compatibility property: the AA boundary grey anchor is preserved exactly.

%% =====================================================================
\section{Symmetry}

OKCA is \textbf{symmetric}: $\mathrm{okca}(A, B) = \mathrm{okca}(B, A)$.
This follows from:
...
\begin{enumerate}
  ...
  \item The ratio formula
    $(Y_{\mathrm{lighter}} + 0.05)\,/\,(Y_{\mathrm{darker}} + 0.05)$
    uses positional roles determined by~$L$.
\end{enumerate}
...
Y_{\mathrm{WCAG}}$ for warm hues, so OKCA underestimates those pairs
...
\texttt{A\_THRESH}  & 0.05  & Oklab $a$ gate: green correction fires only when $a < -0.05$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{description}[style=nextline,leftmargin=2em]
  \item[$C_{\mathrm{THRESH}} = 0.15$]
    Typical Oklab chroma for designer palette saturated colours; lightly
    tinted neutrals ($C < 0.05$) receive ${<}\,10\%$ of the full penalty.

  \item[$\mathrm{CHROMA\_K} = 0.75$]
    Yields $\mathit{exp} = 1.75$ at full saturation, which reduces hot
    pink's effective ratio from $6.6$ to $4.0$ (below the $4.5$ AA
    threshold).  This is the target: WCAG's two most-cited false passes
    (hot pink and dark orange on near-black) become OKCA false failures.

  \item[$A_{\mathrm{THRESH}} = 0.05$]
    Separates true greens ($a \approx -0.13$ for forest green) from
    blue-shifted colours like Tailwind blue-700 ($a \approx -0.047$).
    The gate must be above $-0.047$ and below $-0.085$ (the weakest
    green that needs correction).

  \item[$K_{\mathrm{DARK}} = 0.155$]
    Minimum value such that white/\texttt{\#228b22} gives a raw ratio
    $\le 4.45$ (rounds to $4.4$, not $4.5$).  Derived analytically from
    the target $\mathit{darkerY} \ge 0.18596$.
\end{description}
...
WCAG~AA ($4.5{:}1$) applies uniformly regardless of text size.  OKCA
outputs a single ratio; size-dependent thresholds are the caller's
responsibility.
